...
const [todoist_credentials, usajobs_credentials] = require("./auth/credentials.json");

const client_id = process.env.CLIENT_ID || todoist_credentials.client_id;
const client_secret = process.env.CLIENT_SECRET || todoist_credentials.client_secret;
const scope = "data:read_write";

const user_agent = process.env.USER_AGENT || usajobs_credentials["User-Agent"];
const authorization_key = process.env.AUTHORIZATION_KEY || usajobs_credentials["Authorization-Key"];
...
function request_handler(req, res) {
    ...
    if (req.url === "/") {
		...
		sessions.set(state, {description, location});
...
function redirect_to_todoist(state, res) {
    ...
    let uri = new URLSearchParams({state, client_id, scope}).toString();
    console.log(uri);
...
function get_jobs_information(user_input, access_token, res) {
    ...
    const usajobs_request = https.request(jobs_endpoint, options);
...
function receive_list_response(body, jobs, access_token, res) {
    const {id: parent_id} = JSON.parse(body);
    create_jobs(jobs, parent_id, access_token, res);
}

function create_jobs(jobs, parent_id, access_token, res) {
    const task_endpoint = "https://api.todoist.com/rest/v2/tasks";
    const options = {
        method: "POST",
        headers: {
    ...
    function receive_task_response(body, res) {
        ...
        if (tasks_added_count === jobs.length) {
            res.writeHead(302, {Location: `https://todoist.com/showTask?id=${parent_id}`}).end();
        }
    }
}
